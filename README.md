...
Проект открытый — **любой может улучшить код или предложить идею!**

- 🐛 **Нашёл баг / есть идея?** → открой [Issue](../../issues) (даже «не работает, вот скрин» помогает)
- 🔧 **Хочешь починить сам?** → форкни, поправь, пришли Pull Request
- 📋 **Что можно сделать** → список багов и идей в [KNOWN_ISSUES.md](KNOWN_ISSUES.md) (есть метки 🟢 для новичков)
- 📖 **Как начать** → гайд для контрибьюторов в [CONTRIBUTING.md](CONTRIBUTING.md)

Это v1.0 от одного человека — баги есть, и это нормально. Если подхватишь — будет супер. ⭐ Звезда репозиторию тоже помогает!
